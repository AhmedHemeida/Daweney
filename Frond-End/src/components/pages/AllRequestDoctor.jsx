...
export default function AllRequestDoctor() {
  ...
  let goToAcc = (datauser) => {
    navigate("/accpatient", { state: datauser });
    // navigate("/accpatient" );
  };

  let apply = (datauser) => {
    let data = {
      reqId:datauser.RequestId , 
      customerId : datauser.customerId ,
      providerId : datauser.providerId ,
      typeofservice:datauser.typeofservice ,   
      priceOfService:50
    }
            ...
            <div key={user.id} className="col-md-4">
              <div className="card p-2">
                <div onClick={() => goToAcc(user)}>
                  ...
                  <div className="d-flex flex row align-items-center mr-2 mt-2">
                    <div className="nursing1">
                      {" "}
                      <span className="nursing">Name : </span>
                      {user.name}
                    </div>
                    <div className="nursing1">
                      {" "}
                      <span className="nursing">Address : </span>
                      {user.address}{" "}
                    </div>
                    <div className="nursing1">
                      {" "}
                ...
                <div className="buttons">
                  <button className="r4" onClick={() => apply(user)} >Apply</button>
                  <button className="r5" onClick={() => cancel(user)}>Cancel</button>
                </div>
              </div>
            </div>
          );
